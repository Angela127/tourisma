...
# 'State', 'Year', and 'Domestic tourism expenditure (RM million)'
cols = ["State", "Year", "Domestic tourism expenditure (RM million)"]
df = pd.read_excel(EXCEL_PATH, usecols=cols)

# Ensure data types and clean states
df["Year"] = df["Year"].astype(int)
df["Domestic tourism expenditure (RM million)"] = pd.to_numeric(
    df["Domestic tourism expenditure (RM million)"], errors="coerce"
)
...
# Sort states by their latest (2025) expenditure volume so the legend entries
# match the visual top-to-bottom order of the curves on the right side.
latest_year = df["Year"].max()
sorted_states = (
    df[df["Year"] == latest_year]
    .sort_values("Domestic tourism expenditure (RM million)", ascending=False)["State"]
    .tolist()
)
...
for i, state in enumerate(sorted_states):
    ...
    plt.plot(
        state_data["Year"],
        state_data["Domestic tourism expenditure (RM million)"],
...
plt.title(
    "Yearly Trend of Domestic Tourism Expenditure by State (2019–2025)",
    fontsize=15,
    fontweight="bold",
    pad=15
)
...
plt.ylabel("Domestic Tourism Expenditure (RM Million)", fontsize=12, labelpad=10)
...
plt.legend(
    ...
    fontsize=9.5,
    frameon=True,
    title="States (by 2025 Expenditure)"
)
plt.tight_layout()

# Save high-resolution trend figure
trend_output_path = BASE_DIR / "domestic_tourism_expenditure_trend.png"
plt.savefig(trend_output_path, dpi=300, bbox_inches="tight")
print(f"Saved trend graph to: {trend_output_path}")
...
fig.suptitle(
    "Domestic Tourism Expenditure by State (2019–2025)",
    fontsize=15,
    fontweight="bold",
    y=0.98
)
...
fig.supylabel("Domestic Tourism Expenditure (RM Million)", fontsize=11, fontweight="bold", x=0.01)
...
# Save high-resolution grid figure
grid_output_path = BASE_DIR / "domestic_tourism_expenditure_grid.png"
plt.savefig(grid_output_path, dpi=300, bbox_inches="tight")
print(f"Saved grid graph to: {grid_output_path}")
